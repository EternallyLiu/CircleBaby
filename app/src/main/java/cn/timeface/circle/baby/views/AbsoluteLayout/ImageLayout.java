 ...
 */
package cn.timeface.circle.baby.views.AbsoluteLayout;
 ...
 * deprecated AbsoluteLayout.<br/>
 * 可以按照相对与背景图片的像素值,放置子View
 * 而且背景图片可以按照可用的空间来调整。
 */
public class ImageLayout extends ViewGroup {
     ...
     * Changes the background image and its layout dimensions.
     */
    public void setImageResource(Bitmap urlBitmap, int imageWidth, int imageHeight) {
        bitmap = urlBitmap;
        bitmapSrcRect = bitmapRect(bitmap);
        this.imageWidth = imageWidth;
        this.imageHeight = imageHeight;
    ...
    private boolean activeKeyBord = true;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!activeKeyBord) {
            setMeasuredDimension(oldW, oldHeight);
            return;
        }
    ...
    public boolean isActiveKeyBord() {
        return activeKeyBord;
    }

    public void setActiveKeyBord(boolean activeKeyBord) {
        this.activeKeyBord = activeKeyBord;
    }
