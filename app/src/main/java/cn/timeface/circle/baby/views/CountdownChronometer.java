...
public class CountdownChronometer extends Chronometer {
    ...
    private synchronized boolean updateText(long now) {
        ...
        for (int i = 0; i < text.length(); i++) {
            int c = text.charAt(i);
            if (c >= 48 && c <= 57) {
                msb.setSpan(new ForegroundColorSpan(Color.parseColor("#ff3232")), i, i + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
...
//            msb.clear();
//            msb.append(text);
//            msb.setSpan(new ForegroundColorSpan(Color.parseColor("#f7993d")), 2, msb.length() - 3, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
//            setTextAndEvent(msb);
//        }
//        else
//        {
//            setTextAndEvent(text);
//        }
